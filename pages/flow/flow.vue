<template>
<view class="flex-col justify-start relative page">
  <view class="flex-col justify-start relative">
    <view class="section"></view>
    <view class="flex-col section_2 pos">
      <view class="flex-row justify-between items-center group">
        <text class="font text">9:41</text>
        <view class="flex-row">
          <image
            class="image"
            src="/static/flow/de45c4b9200cbb0f4c6112808628a9bc.png"
          />
          <image
            class="image_2 ml-5"
            src="/static/flow/6c2df855d6b0741d9025015df39ee098.png"
          />
          <image
            class="image_3 ml-5"
            src="/static/flow/07d18b8046c5af20976cc8363e5c8fa8.png"
          />
        </view>
      </view>
      <view class="flex-row justify-center relative group_3">
        <image
          class="image_4 pos_3"
          src="/static/flow/859c9731589073958cdfbdfd1b6dd60b.png"
        />
        <text class="text_2">服务流程</text>
      </view>
      <view class="flex-row justify-center items-center group_2 pos_2">
        <view class="flex-row justify-start items-center image-wrapper">
          <image
            class="image_6"
            src="/static/flow/ff465c47eaf7a5dc580be9526a79f1cc.png"
          />
        </view>
        <view class="section_3 ml-11"></view>
        <view class="flex-row justify-start items-center image-wrapper ml-11">
          <image
            class="image_5"
            src="/static/flow/cf3676537609a5ebdbd85e509fe5924a.png"
          />
        </view>
      </view>
    </view>
    <view class="flex-col section_4 pos_5">
      <view class="flex-row self-stretch group_4">
        <view class="flex-col self-start group_5">
          <view class="relative section_6"></view>
          <view class="relative section_6 mt-33"></view>
          <view class="section_6 mt-33"></view>
        </view>
        <view class="flex-col justify-start items-end flex-1 relative ml-9">
          <view class="flex-col justify-start items-end relative section_5">
            <image
              class="image_9"
              src="/static/flow/9b8d4c1a090e53ab2c7641efd0256b7c.png"
            />
            <image
              class="image_8 pos_6"
              src="/static/flow/3e350b33d13439b4a4a0908b248250bf.png"
            />
            <view class="flex-col justify-start items-start image-wrapper_3 pos_11">
              <image
                class="image_13"
                src="/static/flow/fea4abbb07e78bccf8938b2861008fd2.png"
              />
            </view>
            <image
              class="image_12 pos_9"
              src="/static/flow/47cea1a068240003952eb9f4d96a4e87.png"
            />
            <view class="section_7 pos_10"></view>
            <image
              class="image_10 pos_8"
              src="/static/flow/0844a207576860c67627226fd7c3cc2b.png"
            />
            <view class="flex-col justify-start items-center image-wrapper_2 pos_7">
              <image
                class="image_11"
                src="/static/flow/95c382cd9e6f83e0fe45328fa82aed01.png"
              />
            </view>
          </view>
          <view class="flex-col justify-start items-start text-wrapper button">
            <text class="font text_3 text_4">全程一对一</text>
          </view>
          <view class="flex-col justify-start items-start text-wrapper_2 button">
            <text class="font text_3">数据库支持</text>
          </view>
          <view class="flex-col justify-start items-start text-wrapper_3 button">
            <text class="font text_3 text_5">无条件修改</text>
          </view>
        </view>
      </view>
      <view class="flex-col self-stretch relative section_8">
        <view class="flex-row items-center self-stretch">
          <image
            class="image_14"
            src="/static/flow/2e2021725d797d829d1cfa472938e479.png"
          />
          <text class="font_2 text_6 text_7 ml-5">赵*（昵称）等128人已免费咨询</text>
        </view>
        <view class="self-center group_6">
          <text class="font_3 text_6 text_8">快速响应</text>
          <text class="font_3 text_6 text_9">免费咨询</text>
        </view>
        <view class="flex-col button_2 section_9">
          <text class="self-center font_4 text_6 text_10">立即咨询</text>
          <view class="flex-col justify-start items-center self-stretch section_10 mt-9">
            <view class="group_7">
              <text class="font_2 text_6 text_11">免费</text>
              <text class="font_5 text_6 text_12">获取标书报价</text>
            </view>
          </view>
        </view>
      </view>
      <image
        class="shrink-0 self-center image_15"
        src="/static/flow/f821dc848b076722d2ff3e1a83daef15.png"
      />
      <view class="flex-row items-center self-stretch relative section_11">
        <view class="flex-col items-start flex-1">
          <text class="font_3 text_13">优质的服务流程</text>
          <text class="font_6 text_14">环环相扣的服务流程每一步</text>
          <text class="font_6 text_15">都为中标保驾护航</text>
        </view>
        <view class="ml-20 flex-col shrink-0 group_8">
          <view class="flex-col justify-start items-center relative group_9">
            <image
              class="image_17"
              src="/static/flow/d42a8242df634ed713675a247c59a80b.png"
            />
            <image
              class="image_16 pos_12"
              src="/static/flow/bf61d474a718a7aaf9a2eb15facbfda5.png"
            />
          </view>
          <view class="shrink-0 relative section_12"></view>
        </view>
      </view>
      <image
        class="shrink-0 self-center image_15 image_18"
        src="/static/flow/f821dc848b076722d2ff3e1a83daef15.png"
      />
    </view>
    <image
      class="image_7 pos_4"
      src="/static/flow/803913e5ae6a717d9199f6cd1f03164d.png"
    />
    <view class="flex-col section_13 pos_13">
      <view class="flex-col justify-start items-center self-center text-wrapper_4">
        <text class="font_7 text_16">step 1</text>
      </view>
      <view class="flex-row items-center self-stretch group_10">
        <view class="flex-col justify-start items-center relative group_11">
          <image
            class="image_19"
            src="/static/flow/ef08f45c351310a6736debd4f4c8ccdd.png"
          />
          <image
            class="image_20 pos_14"
            src="/static/flow/cf2742967423a713452810f10ff00cc4.png"
          />
          <view class="section_14 pos_15"></view>
        </view>
        <view class="ml-24 flex-col items-start group_12">
          <text class="font_4 text_17">新建服务</text>
          <text class="mt-26 font_8 text_18">输入并提交个人资料与招标文件资料</text>
        </view>
      </view>
      <view class="flex-col justify-start items-center text-wrapper_5 text-wrapper_4">
        <text class="font_7 text_16">step 2</text>
      </view>
      <view class="flex-row justify-between items-center self-stretch group_13">
        <view class="flex-col text_18">
          <text class="self-start font_4 text_19">资料审核</text>
          <text class="mt-26 self-stretch font_8">解析招标文件，提供资料准备清单</text>
        </view>
        <view class="flex-col justify-start items-center relative group_14">
          <image
            class="image_21"
            src="/static/flow/330996f196aa92fe9629b9b10bf796e4.png"
          />
          <image
            class="image_22 pos_16"
            src="/static/flow/99215d75e5f87b8d9b835b1b0264af07.png"
          />
          <image
            class="image_23 pos_17"
            src="/static/flow/83fbaf2700429bcec277eb5bd80e906f.png"
          />
          <view class="section_15 pos_18"></view>
        </view>
      </view>
      <view class="flex-col self-stretch group_15">
        <view class="flex-col justify-start items-center text-wrapper_5 text-wrapper_4">
          <text class="font_7 text_16">step 3</text>
        </view>
        <view class="flex-row justify-between items-center self-stretch group_16 mt-25">
          <view class="flex-col justify-start relative group_17">
            <view class="flex-col justify-start items-center relative section_16">
              <image
                class="image_30"
                src="/static/flow/f2e5b5eaf136a657529f3c5d25d1a559.png"
              />
              <image
                class="image_29 pos_24"
                src="/static/flow/6ae26af2b69dae5832f91c00a693a1d6.png"
              />
            </view>
            <image
              class="image_24 pos_19"
              src="/static/flow/534ce6651db251cdb94a7b0ad3425a59.png"
            />
            <image
              class="image_25 pos_20"
              src="/static/flow/b63334c3a767cae71d074ca13864829d.png"
            />
            <image
              class="image_27 pos_22"
              src="/static/flow/620d1c31a73bef2d0f036977c9ce36fa.png"
            />
            <image
              class="image_28 pos_23"
              src="/static/flow/9c12924c5f094ab18d8d91ac3ce5f1d7.png"
            />
            <image
              class="image_26 pos_21"
              src="/static/flow/33e59fb99fb1f3e58b0bac08507c65fc.png"
            />
          </view>
          <view class="flex-col group_18">
            <text class="self-start font_4 text_20">服务沟通</text>
            <text class="mt-26 self-stretch font_8">根据招标评分内容细节沟通，全面了解</text>
          </view>
        </view>
        <view class="flex-col self-stretch mt-25">
          <view class="flex-col justify-start items-center text-wrapper_5 text-wrapper_4">
            <text class="font_7 text_16">step 4</text>
          </view>
          <view class="mt-22 flex-row items-center self-stretch">
            <view class="flex-col group_20">
              <text class="self-start font_4 text_21">标书制作</text>
              <view class="mt-26 flex-col items-start self-stretch">
                <text class="font_8">支付预付款后</text>
                <text class="font_8 mt-3">审查核对资料进行制作</text>
              </view>
            </view>
            <view class="flex-col justify-start relative group_19 ml-11">
              <view class="flex-col justify-start items-center relative section_17">
                <view class="flex-col justify-start items-end image-wrapper_4">
                  <image
                    class="image_32"
                    src="/static/flow/7e387f5791f34e3ceaeec5041264cf00.png"
                  />
                </view>
                <image
                  class="image_37 pos_30"
                  src="/static/flow/1b21cc5bad72c01905df781b069743d4.png"
                />
                <image
                  class="image_35 pos_28"
                  src="/static/flow/722991e7656dcbab76a31e71a162b370.png"
                />
                <image
                  class="image_38 pos_31"
                  src="/static/flow/1cdc90c5d6cb523fb61ab2682db07218.png"
                />
                <image
                  class="image_36 pos_29"
                  src="/static/flow/d5aa41763582d4050bcc989a0a8dfc7e.png"
                />
                <image
                  class="image_34 pos_27"
                  src="/static/flow/b31336079ff1ab4e8561fe57c6aa151d.png"
                />
                <image
                  class="image_39 pos_32"
                  src="/static/flow/a52fa02b4510ddcff3ab96b1c300020e.png"
                />
                <image
                  class="image_33 pos_26"
                  src="/static/flow/aae8a79a18303fdbc4d9c501fb75f605.png"
                />
              </view>
              <image
                class="image_31 pos_25"
                src="/static/flow/5bdc59d95e2d1761423e34475083f870.png"
              />
              <view class="section_18 pos_33"></view>
            </view>
          </view>
        </view>
      </view>
      <view class="flex-col self-stretch group_21">
        <view class="flex-col justify-start items-center text-wrapper_5 text-wrapper_4">
          <text class="font_7 text_16">step 5</text>
        </view>
        <view class="mt-22 flex-row justify-between items-center self-stretch">
          <view class="flex-col justify-start relative group_14">
            <view class="flex-col justify-start items-start image-wrapper_5">
              <image
                class="image_40"
                src="/static/flow/7d06af0d5ff05056fc3ccc203b130404.png"
              />
            </view>
            <image
              class="image_42 pos_35"
              src="/static/flow/34551fe31404e0fedc88934dd1589658.png"
            />
            <image
              class="image_41 pos_34"
              src="/static/flow/73df7d23a4d6c812364b8fa30f88147f.png"
            />
            <view class="section_19 pos_36"></view>
          </view>
          <view class="flex-col group_22">
            <text class="self-start font_4 text_22">验收付尾款</text>
            <text class="mt-26 self-stretch font_8">文件截图验收，无误后支付剩余尾款</text>
          </view>
        </view>
      </view>
      <view class="flex-col self-stretch group_23">
        <view class="flex-col justify-start items-center text-wrapper_5 text-wrapper_4">
          <text class="font_7 text_16">step 6</text>
        </view>
        <view class="mt-22 flex-col self-stretch">
          <view class="flex-row justify-between items-center">
            <view class="flex-col text_18">
              <text class="self-start font_4 text_23">无忧售后</text>
              <text class="mt-26 self-stretch font_8">提醒注意事项，提供后期更改和追踪服务</text>
            </view>
            <view class="flex-col items-end relative group_24">
              <image
                class="image_43"
                src="/static/flow/0d223e123c845bd3a7f1ad545b527d5d.png"
              />
              <image
                class="image_48"
                src="/static/flow/72bb6ab609b5f7cbdae950537b82597f.png"
              />
              <image
                class="image_45 pos_38"
                src="/static/flow/f23ddb6ebb38325c20f04436892b9eea.png"
              />
              <image
                class="image_44 pos_37"
                src="/static/flow/a183d3f99baead910093973209eb9973.png"
              />
              <image
                class="image_46 pos_39"
                src="/static/flow/70996c4a59b764ce486755e8b0fc8666.png"
              />
              <image
                class="image_47 pos_40"
                src="/static/flow/ed5c45f12c250e5f64a51f1a29cf171c.png"
              />
            </view>
          </view>
          <view class="mt-22 flex-row justify-end items-center">
            <image
              class="image_49"
              src="/static/flow/c3f702eba32d2b19e509b22707b97480.png"
            />
            <text class="ml-6 font_5 text_24">了解流程详情</text>
          </view>
        </view>
      </view>
    </view>
  </view>
  <view class="flex-col justify-start items-center button_3 text-wrapper_6">
    <text class="text_25">即刻免费咨询</text>
  </view>
</view>
</template>

<script setup>

</script>

<style scoped lang="scss">
.mt-33 {
  margin-top: 66rpx;
}
.ml-9 {
  margin-left: 18rpx;
}
.mt-9 {
  margin-top: 18rpx;
}
.mt-25 {
  margin-top: 50rpx;
}
.mt-3 {
  margin-top: 6rpx;
}
.ml-11 {
  margin-left: 22rpx;
}
.ml-5 {
  margin-left: 10rpx;
}
.page {
  background-color: #ffffff;
  background-image: linear-gradient(180deg, #0584f6 0%, #58b5ff 19.9%, #ffffff 50.9%);
  width: 100%;
  overflow-y: auto;
  overflow-x: hidden;
  height: 100%;
  .section {
    background-image: linear-gradient(180deg, #0584f6 0%, #58b5ff 19.9%, #ffffff 50.9%);
    height: 3794rpx;
  }
  .section_2 {
    padding: 36rpx 28rpx 28rpx 34rpx;
    background-color: #ffffff00;
    backdrop-filter: blur(27.18rpx);
    .group {
      padding-left: 32rpx;
      .text {
        font-size: 30rpx;
        font-weight: 800;
        line-height: 22.26rpx;
      }
      .image {
        width: 34rpx;
        height: 22rpx;
      }
      .image_2 {
        width: 30rpx;
        height: 22rpx;
      }
      .image_3 {
        width: 48rpx;
        height: 22.66rpx;
      }
    }
    .group_3 {
      margin-top: 52rpx;
      .image_4 {
        width: 18rpx;
        height: 34rpx;
      }
      .pos_3 {
        position: absolute;
        left: 0;
        top: 50%;
        transform: translateY(-50%);
      }
      .text_2 {
        color: #ffffff;
        font-size: 34rpx;
        font-family: PingFang SC;
        font-weight: 700;
        line-height: 31.9rpx;
        opacity: 0.9;
      }
    }
    .group_2 {
      padding: 12rpx 24rpx;
      border-radius: 50rpx;
      width: 174rpx;
      border-left: solid 1rpx #96969633;
      border-right: solid 1rpx #96969633;
      border-top: solid 1rpx #96969633;
      border-bottom: solid 1rpx #96969633;
      .image-wrapper {
        width: 38rpx;
        .image_6 {
          width: 38rpx;
          height: 14rpx;
        }
        .image_5 {
          width: 34rpx;
          height: 34rpx;
        }
      }
      .section_3 {
        background-color: #00000033;
        width: 1rpx;
        height: 37rpx;
      }
    }
    .pos_2 {
      position: absolute;
      right: 12rpx;
      top: 100rpx;
    }
  }
  .pos {
    position: absolute;
    left: 0;
    right: 0;
    top: 0;
  }
  .section_4 {
    padding-left: 38rpx;
    padding-bottom: 328rpx;
    background-image: url('/static/flow/3fb0468a15c70c8f3f454eae6421d073.png');
    background-size: 100% 100%;
    background-repeat: no-repeat;
    height: 1302rpx;
    .group_4 {
      padding-left: 6rpx;
      .group_5 {
        margin-top: 118rpx;
        width: 20rpx;
        .section_6 {
          background-color: #fcd52c;
          box-shadow: 0rpx 0rpx 4rpx #00000026;
          border-radius: 50%;
          height: 20rpx;
        }
      }
      .section_5 {
        padding: 36rpx 0 236rpx;
        background-image: url('/static/flow/0c23074d2c452985c3e1d3f5e11c1a4f.png');
        background-size: 100% 100%;
        background-repeat: no-repeat;
        width: 478rpx;
        .image_9 {
          margin-right: 16rpx;
          width: 411.4rpx;
          height: 270.66rpx;
        }
        .image_8 {
          width: 216rpx;
          height: 314rpx;
        }
        .pos_6 {
          position: absolute;
          right: 120.2rpx;
          top: 32.1rpx;
        }
        .image-wrapper_3 {
          padding-top: 44rpx;
          background-image: url('/static/flow/e5950d7e9bcb8af73b9d35332a9b0c62.png');
          background-size: 100% 100%;
          background-repeat: no-repeat;
          width: 316rpx;
          height: 224rpx;
          .image_13 {
            margin-left: 36rpx;
            width: 30rpx;
            height: 180rpx;
          }
        }
        .pos_11 {
          position: absolute;
          left: 61.3rpx;
          top: 50%;
          transform: translateY(-50%);
        }
        .image_12 {
          width: 52rpx;
          height: 52rpx;
        }
        .pos_9 {
          position: absolute;
          left: 85.2rpx;
          top: 127.82rpx;
        }
        .section_7 {
          background-color: #263238;
          width: 42.94rpx;
          height: 41.98rpx;
        }
        .pos_10 {
          position: absolute;
          left: 60.26rpx;
          top: 161.12rpx;
        }
        .image_10 {
          width: 98rpx;
          height: 98rpx;
        }
        .pos_8 {
          position: absolute;
          left: 118.68rpx;
          top: 51.34rpx;
        }
        .image-wrapper_2 {
          padding: 8rpx 0;
          background-image: url('/static/flow/fd7cf62a910c048a81c7e4bdb40c0ffd.png');
          background-size: 100% 100%;
          background-repeat: no-repeat;
          width: 102rpx;
          .image_11 {
            width: 84rpx;
            height: 84rpx;
          }
        }
        .pos_7 {
          position: absolute;
          left: 116.82rpx;
          top: 49.56rpx;
        }
      }
      .text-wrapper {
        position: absolute;
        left: 0;
        top: 92rpx;
        .text_4 {
          line-height: 25.82rpx;
        }
      }
      .button {
        padding: 20rpx 0;
        background-image: linear-gradient(90deg, #ffffff80 0%, #ffffff00 100%);
        border-radius: 58rpx;
        width: 232rpx;
      }
      .text-wrapper_2 {
        position: absolute;
        left: 0;
        top: 178rpx;
      }
      .text_3 {
        margin-left: 20rpx;
      }
      .text-wrapper_3 {
        position: absolute;
        left: 0;
        top: 264rpx;
        .text_5 {
          line-height: 26.34rpx;
        }
      }
    }
    .section_8 {
      margin-right: 40rpx;
      margin-top: -154rpx;
      padding: 20rpx 32rpx 36rpx;
      background-color: #ffffff;
      border-radius: 16rpx;
      box-shadow: 0rpx 34rpx 32rpx -18rpx #00233f40;
      .image_14 {
        width: 100rpx;
        height: 40rpx;
      }
      .font_2 {
        font-size: 24rpx;
        font-family: PingFang SC;
        line-height: 22.46rpx;
        font-weight: 700;
      }
      .text_6 {
        text-transform: capitalize;
      }
      .text_7 {
        color: #0000004d;
        line-height: 22.58rpx;
      }
      .group_6 {
        margin-top: 28rpx;
        line-height: 37.52rpx;
        .text_8 {
          color: #333333;
          line-height: 37.4rpx;
        }
        .text_9 {
          color: #fa9014;
        }
      }
      .button_2 {
        align-self: center;
        margin-top: 36rpx;
        .text_10 {
          color: #ffffff;
          line-height: 33.52rpx;
        }
        .section_10 {
          padding: 8rpx 0;
          background-color: #fcb053;
          border-radius: 4rpx;
          width: 248rpx;
          .group_7 {
            line-height: 22.46rpx;
            .text_11 {
              color: #ffffff;
            }
            .text_12 {
              color: #ffffff;
              line-height: 22.22rpx;
            }
          }
        }
      }
      .section_9 {
        padding: 24rpx 106rpx 14rpx;
        background-image: linear-gradient(90deg, #fd8b27 0%, #fe6112 100%);
        border-radius: 126rpx;
        box-shadow: 0rpx 0rpx 8rpx #fe6413;
      }
    }
    .image_15 {
      width: 510rpx;
      height: 40rpx;
    }
    .section_11 {
      margin-right: 42rpx;
      margin-top: -12rpx;
      padding: 28rpx 32rpx 12rpx;
      background-color: #ffffff;
      border-radius: 16rpx;
      box-shadow: 0rpx 46rpx 58rpx -22rpx #0061af40;
      .text_13 {
        color: #1890ff;
      }
      .font_6 {
        font-size: 28rpx;
        font-family: PingFang SC;
        line-height: 34rpx;
        color: #00000080;
      }
      .text_14 {
        margin-top: 32rpx;
      }
      .text_15 {
        margin-top: 8rpx;
      }
      .group_8 {
        width: 215.4rpx;
        .group_9 {
          padding-top: 28rpx;
          .image_17 {
            width: 110rpx;
            height: 112rpx;
          }
          .image_16 {
            width: 48rpx;
            height: 50rpx;
          }
          .pos_12 {
            position: absolute;
            right: 21.02rpx;
            top: 0;
          }
        }
        .section_12 {
          margin-top: -8rpx;
          background-color: #ebebeb;
          border-radius: 50%;
          height: 16.3rpx;
        }
      }
    }
    .font_3 {
      font-size: 40rpx;
      font-family: PingFang SC;
      line-height: 37.52rpx;
      font-weight: 700;
    }
    .image_18 {
      margin-top: -8rpx;
    }
  }
  .pos_5 {
    position: absolute;
    left: 0;
    right: 0;
    top: 250rpx;
  }
  .font {
    font-size: 28rpx;
    font-family: PingFang SC;
    line-height: 26.2rpx;
    font-weight: 700;
    color: #ffffff;
  }
  .image_7 {
    filter: drop-shadow(2rpx 4rpx 2rpx #05177540);
    width: 622rpx;
    height: 114rpx;
  }
  .pos_4 {
    position: absolute;
    left: 54.62rpx;
    top: 184rpx;
  }
  .section_13 {
    padding: 40rpx 16rpx 40rpx 20rpx;
    background-color: #ffffff;
    border-radius: 16rpx;
    filter: drop-shadow(0rpx 46rpx 29rpx #0061af40);
    .text-wrapper_4 {
      padding: 16rpx 0;
      background-image: linear-gradient(90deg, #fd8b27 0%, #fe6112 100%);
      border-radius: 50rpx;
      filter: drop-shadow(0rpx 0rpx 4rpx #fe6413);
      width: 194rpx;
    }
    .group_10 {
      padding: 44rpx 20rpx 60rpx;
      .group_11 {
        padding-bottom: 16rpx;
        flex: 1 1 267.56rpx;
        .image_19 {
          width: 267.56rpx;
          height: 184.54rpx;
        }
        .image_20 {
          width: 192rpx;
          height: 168rpx;
        }
        .pos_14 {
          position: absolute;
          left: 50%;
          bottom: 5.56rpx;
          transform: translateX(-50%);
        }
        .section_14 {
          background-color: #f5f5f5;
          border-radius: 50%;
          width: 207.52rpx;
          height: 12.16rpx;
        }
        .pos_15 {
          position: absolute;
          left: 50%;
          top: 187.84rpx;
          transform: translateX(-50%);
        }
      }
      .group_12 {
        flex: 1 1 267.56rpx;
        .text_17 {
          line-height: 33.76rpx;
        }
      }
    }
    .text-wrapper_5 {
      align-self: center;
    }
    .font_7 {
      font-size: 44rpx;
      font-family: PingFang SC;
      line-height: 34rpx;
      font-weight: 700;
      color: #ffffff;
    }
    .text_16 {
      line-height: 32.64rpx;
      text-transform: uppercase;
    }
    .group_13 {
      margin-top: 44rpx;
      padding-left: 40rpx;
      padding-right: 28rpx;
    }
    .text_18 {
      width: 250rpx;
      .text_19 {
        line-height: 33.84rpx;
      }
      .text_23 {
        line-height: 33.66rpx;
      }
    }
    .font_8 {
      font-size: 28rpx;
      font-family: PingFang SC;
      line-height: 34rpx;
      font-weight: 700;
      color: #666666;
    }
    .group_15 {
      margin-top: 60rpx;
      padding-left: 40rpx;
      padding-right: 24rpx;
      .group_16 {
        padding: 0 4rpx;
        .group_17 {
          padding-top: 104rpx;
          width: 172rpx;
          .section_16 {
            padding: 28rpx 0 20rpx;
            background-image: url('/static/flow/e08f0b99f285728632a724d2faf27300.png');
            background-size: 100% 100%;
            background-repeat: no-repeat;
            width: 172rpx;
            .image_30 {
              width: 128rpx;
              height: 50rpx;
            }
            .image_29 {
              width: 22rpx;
              height: 40rpx;
            }
            .pos_24 {
              position: absolute;
              right: 7.8rpx;
              top: 14.24rpx;
            }
          }
          .image_24 {
            width: 78rpx;
            height: 178rpx;
          }
          .pos_19 {
            position: absolute;
            right: 37.8rpx;
            top: 0;
          }
          .image_25 {
            width: 57.78rpx;
            height: 72.38rpx;
          }
          .pos_20 {
            position: absolute;
            left: 8.12rpx;
            top: 10.94rpx;
          }
          .image_27 {
            width: 66rpx;
            height: 92rpx;
          }
          .pos_22 {
            position: absolute;
            left: 50%;
            bottom: 48.2rpx;
            transform: translateX(-50%);
          }
          .image_28 {
            width: 26rpx;
            height: 38rpx;
          }
          .pos_23 {
            position: absolute;
            left: 8.12rpx;
            top: 50%;
            transform: translateY(-50%);
          }
          .image_26 {
            width: 50rpx;
            height: 20rpx;
          }
          .pos_21 {
            position: absolute;
            right: 3.26rpx;
            top: 7.44rpx;
          }
        }
        .group_18 {
          width: 266.98rpx;
          .text_20 {
            line-height: 33.7rpx;
          }
        }
      }
      .group_20 {
        flex: 1 1 277.4rpx;
        .text_21 {
          line-height: 33.58rpx;
        }
      }
      .group_19 {
        padding-bottom: 12rpx;
        flex: 1 1 277.4rpx;
        .section_17 {
          padding: 12rpx 0 8rpx;
          background-image: url('/static/flow/a91848124291d2faf6f4395692a48da9.png');
          background-position: 0% 0%;
          background-size: 268.8rpx 186.26rpx;
          background-repeat: no-repeat;
          .image-wrapper_4 {
            padding: 60rpx 0 68rpx;
            background-image: url('/static/flow/71360749458c77a9f437e1da6640b17e.png');
            background-size: 100% 100%;
            background-repeat: no-repeat;
            width: 190rpx;
            .image_32 {
              margin-right: 20rpx;
              width: 74.08rpx;
              height: 38.72rpx;
            }
          }
          .image_37 {
            width: 128rpx;
            height: 70.46rpx;
          }
          .pos_30 {
            position: absolute;
            right: 21.36rpx;
            bottom: 4.42rpx;
          }
          .image_35 {
            width: 26.86rpx;
            height: 6.68rpx;
          }
          .pos_28 {
            position: absolute;
            right: 98.84rpx;
            bottom: 74.76rpx;
          }
          .image_38 {
            width: 42rpx;
            height: 2rpx;
          }
          .pos_31 {
            position: absolute;
            right: 64.5rpx;
            bottom: 74.46rpx;
          }
          .image_36 {
            width: 24.62rpx;
            height: 10rpx;
          }
          .pos_29 {
            position: absolute;
            right: 37.96rpx;
            bottom: 74.9rpx;
          }
          .image_34 {
            width: 34rpx;
            height: 44rpx;
          }
          .pos_27 {
            position: absolute;
            left: 50%;
            bottom: 47.68rpx;
            transform: translateX(-50%);
          }
          .image_39 {
            width: 50rpx;
            height: 8rpx;
          }
          .pos_32 {
            position: absolute;
            right: 93.72rpx;
            bottom: 45.6rpx;
          }
          .image_33 {
            width: 36rpx;
            height: 46rpx;
          }
          .pos_26 {
            position: absolute;
            left: 50%;
            bottom: 45.68rpx;
            transform: translateX(-50%);
          }
        }
        .image_31 {
          width: 114rpx;
          height: 166rpx;
        }
        .pos_25 {
          position: absolute;
          left: 24.6rpx;
          bottom: 4.66rpx;
        }
        .section_18 {
          background-color: #f5f5f5;
          border-radius: 50%;
          height: 12.18rpx;
        }
        .pos_33 {
          position: absolute;
          left: 30.16rpx;
          right: 30.16rpx;
          top: 187.82rpx;
        }
      }
    }
    .group_21 {
      margin-top: 60rpx;
      .group_22 {
        margin-right: 44rpx;
        width: 252rpx;
        .text_22 {
          line-height: 33.74rpx;
        }
      }
    }
    .group_14 {
      padding-bottom: 16rpx;
      width: 268.42rpx;
      .image_21 {
        width: 268.42rpx;
        height: 184.52rpx;
      }
      .image_22 {
        width: 150rpx;
        height: 190rpx;
      }
      .pos_16 {
        position: absolute;
        right: 20.84rpx;
        top: 50%;
        transform: translateY(-50%);
      }
      .image_23 {
        width: 176rpx;
        height: 106rpx;
      }
      .pos_17 {
        position: absolute;
        left: 50%;
        bottom: 5.96rpx;
        transform: translateX(-50%);
      }
      .section_15 {
        background-color: #f5f5f5;
        border-radius: 50%;
        width: 208.18rpx;
        height: 12.16rpx;
      }
      .pos_18 {
        position: absolute;
        left: 50%;
        top: 187.84rpx;
        transform: translateX(-50%);
      }
      .image-wrapper_5 {
        padding: 20rpx 0 48rpx;
        background-image: url('/static/flow/03fd5c0002d5ac26deaef491ef7c3580.png');
        background-position: 0% 0%;
        background-size: 268.42rpx 185.98rpx;
        background-repeat: no-repeat;
        width: 268.42rpx;
        .image_40 {
          margin-left: 32rpx;
          opacity: 0;
          width: 170rpx;
          height: 118rpx;
        }
      }
      .image_42 {
        width: 86rpx;
        height: 148rpx;
      }
      .pos_35 {
        position: absolute;
        right: 38.76rpx;
        bottom: 6.16rpx;
      }
      .image_41 {
        width: 156rpx;
        height: 162rpx;
      }
      .pos_34 {
        position: absolute;
        left: 50.7rpx;
        bottom: 5.94rpx;
      }
      .section_19 {
        background-color: #f5f5f5;
        border-radius: 50%;
        height: 12.16rpx;
      }
      .pos_36 {
        position: absolute;
        left: 30.12rpx;
        right: 30.12rpx;
        top: 187.84rpx;
      }
    }
    .group_23 {
      margin-top: 60rpx;
      padding-left: 40rpx;
      .group_24 {
        margin-right: 32rpx;
        padding-left: 12rpx;
        .image_43 {
          width: 208rpx;
          height: 104rpx;
        }
        .image_48 {
          margin-top: 32rpx;
          width: 60rpx;
          height: 64rpx;
        }
        .image_45 {
          width: 126rpx;
          height: 146rpx;
        }
        .pos_38 {
          position: absolute;
          right: 6.72rpx;
          bottom: 0;
        }
        .image_44 {
          width: 50rpx;
          height: 48rpx;
        }
        .pos_37 {
          position: absolute;
          left: 50%;
          top: 23.38rpx;
          transform: translateX(-50%);
        }
        .image_46 {
          width: 146rpx;
          height: 122rpx;
        }
        .pos_39 {
          position: absolute;
          left: 50%;
          bottom: 0;
          transform: translateX(-50%);
        }
        .image_47 {
          width: 42rpx;
          height: 82rpx;
        }
        .pos_40 {
          position: absolute;
          left: 0;
          bottom: 0;
        }
      }
      .image_49 {
        width: 32rpx;
        height: 32rpx;
      }
      .text_24 {
        color: #1890ff;
        line-height: 22.48rpx;
      }
    }
  }
  .pos_13 {
    position: absolute;
    left: 38rpx;
    right: 42rpx;
    top: 1220rpx;
  }
  .font_4 {
    font-size: 36rpx;
    font-family: PingFang SC;
    line-height: 34rpx;
    font-weight: 700;
    color: #333333;
  }
  .font_5 {
    font-size: 24rpx;
    font-family: PingFang SC;
    line-height: 22.46rpx;
  }
  .button_3 {
    position: absolute;
    left: 50%;
    top: 3602rpx;
    transform: translateX(-50%);
    .text_25 {
      color: #f2f2f2;
      font-size: 32rpx;
      font-family: PingFang SC;
      font-weight: 700;
      line-height: 30.04rpx;
    }
  }
  .text-wrapper_6 {
    padding: 32rpx 0;
    background-color: #fd6d27;
    border-radius: 58rpx;
    width: 562rpx;
  }
}
</style>